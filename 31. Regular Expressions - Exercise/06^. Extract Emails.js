...
function extracEmails(emails) {

    let pattern = /\s([a-z0-9]+[a-z0-9\_\.\-]*)@[a-z\-]+((\.[a-z]+)+)\b/g;
    let result = emails.match(pattern);
    for (const mails of result) {
        console.log(mails);
    }

}
extracEmails('Just send email to s.miller@mit.edu and j.hopking@york.ac.uk for more information');